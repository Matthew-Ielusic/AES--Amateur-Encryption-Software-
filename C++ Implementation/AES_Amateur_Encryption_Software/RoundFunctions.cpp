...
void RoundFunctions::State::SubBytes() {
    ...
    for (r = 0; r < 4; ++r) {
        for (c = 0; c < 4; ++c) {
            values[r][c] = FiniteField::sBox(values[r][c]);
        }
    }
}

void RoundFunctions::State::ShiftRows()
{
    int r;
    for (r = 1; r < 4; ++r) {
        ...
        int offset = 4 - r;
        memcpy(row,          old + r, offset);
        memcpy(row + offset, old,     r);
    }
}
...
std::vector<uint8_t> RoundFunctions::State::toVector() const
{
    std::vector<uint8_t> output;
    for (int c = 0; c < 4; ++c) {
        for (int r = 0; r < 4; ++r) {
            output.push_back(values[r][c]);
        }
    }
    return output;
}
...
bool RoundFunctions::operator==(const RoundFunctions::State& left, const RoundFunctions::State& right)
{
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
            if (left.values[r][c] != right.values[r][c]) {
                return false;
            }
        }
    }
