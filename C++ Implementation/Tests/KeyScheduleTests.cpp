...
namespace Tests
{
	...
	public:
		...
		{
			std::vector<uint8_t> key = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
			KeySchedule sched = KeySchedule::InverseSchedule(std::vector<uint8_t>(key));
		...
		{
			std::vector<uint8_t> key = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
			KeySchedule sched = KeySchedule::InverseSchedule(key);

			std::vector<uint32_t> expected{ 0xb6630ca6, 0xe13f0cc8, 0xc9ee2589, 0xd014f9a8 };
			for (int i = 0; i < expected.size(); ++i) {
				Assert::AreEqual(expected.at(i), sched.next());
			}
		}
